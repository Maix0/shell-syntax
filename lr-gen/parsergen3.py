...
parser.add_argument(
    "-o", "--output", type=str, help="Python-formatted output of the parsing tables."
)
...
def st_digits(tok, ch):
    if ch.isdigit():
        ...
        advance("digits", "".join(tok.inp), tok.pos, (tok.column + 1, tok.line))
        tok.inp = []
        tok.state = "st_0"
        st_0(tok, ch)
...
def st_string(tok, ch):
    if ch == '"':
        advance("string", "".join(tok.inp), tok.pos, (tok.column, tok.line))
        tok.inp = []
        tok.state = "st_0"
    else:
        tok.inp.append(ch)
...
def print_item(prefix, xxx_todo_changeme, fd=sys.stdout):
    ...
    fd.write(
        "{}{} → {}\n".format(
            prefix, lhs or "⊤", " ".join(rhs[:index] + ["∘"] + rhs[index:])
        )
    )
...
def wfb_rules():
    ...
    for rule, (lhs, rhs) in enumerate(grammar):
        if (rule, len(rhs) - 1) not in wfb_constraints:
            ...
            is_wfb = all(
                (((rule, index) in valign_constraints) or (len(first[rhs[index]]) == 0))
        ...
        for sym in followup(k, seed_lookahead, item):
            ...
            if sym not in tab:
                ...
                if (mapping, sym) in lr_conflicts:
                    ...
                    lr_conflicts[(mapping, sym)] = list(
                        previous(vectors[lr_mapping[tab[sym][1]][0]])
                    ) + [item]
                    had_conflicts.append(sym)
